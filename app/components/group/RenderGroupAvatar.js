...
//     const position = positions[memberDetails.length][index]; // Lấy vị trí theo số lượng thành viên
...
const RenderGroupAvatar = ({ members }) => {
      ...
      const details = await Promise.all(
        members.map(async (memberId) => {
          try {
            ...
            console.error("Error fetching user info for member:", memberId, error);
            return {
              id: memberId,
              fullname: "User",
              urlavatar: null,
  ...
  const displayedAvatars = memberDetails.slice(0, 4).map((member, index) => {
    const position = positions[memberDetails.length]?.[index] || {left: containerSize / 4, top: containerSize / 4}; // Lấy vị trí theo số lượng thành viên
    return member?.urlavatar ? (
      <Image
        key={index}
        source={{ uri: member.urlavatar }}
