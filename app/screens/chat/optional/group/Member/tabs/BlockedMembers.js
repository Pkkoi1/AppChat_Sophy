...
const BlockedMembers = ({ conversation, onConversationUpdate }) => {
  ...
  // Lấy danh sách user bị chặn từ conversation prop (có thể là conversation.data hoặc conversation trực tiếp)
  const blockedUserIds =
    (conversationRef.current?.data && conversationRef.current.data.blocked) ||
    conversationRef.current?.blocked ||
    [];
  ...
  const handleUnblock = useCallback(
    async (userId, fullname) => {
      if (!conversationRef.current?.data.conversationId) {
  ...
  const renderBlockedMember = useCallback(
    ({ item }) => {
      console.log("Rendering blocked member:", item);
      const memberInfo = userInfos[item];
      if (!memberInfo) return null;

      // Đảm bảo rules luôn tồn tại và là object
      const rules =
        (conversationRef.current?.data && conversationRef.current.data.rules) ||
        conversationRef.current?.rules ||
        {};

      const isOwner =
        userInfo?.userId && rules.ownerId
          ? userInfo.userId === rules.ownerId
          : false;
      const isCoOwner =
        userInfo?.userId && Array.isArray(rules.coOwnerIds)
          ? rules.coOwnerIds.includes(userInfo.userId)
          : false;
      const canUnblock = isOwner || isCoOwner;
      console.log(
        `Can unblock: ${canUnblock}, Owner: ${isOwner}, Co-Owner: ${isCoOwner}`
      );
