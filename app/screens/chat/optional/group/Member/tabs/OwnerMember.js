...
const OwnerMember = ({
  ...
  // Filter and sort groupMember to get only owner and coowner
  const sortedOwnerMembers = useMemo(() => {
    return groupMember
      .filter((member) => member.role === "owner" || member.role === "co-owner")
      .sort((a, b) => {
  ...
  useEffect(() => {
    if (sortedOwnerMembers.length === 0) {
      setError("Không có trưởng nhóm hoặc phó nhóm");
    } else {
      setError(null);
    }
  }, [sortedOwnerMembers]);
